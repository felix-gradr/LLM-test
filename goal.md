...
- 1. Adopt and write tests, and run the tests before writing any code. One of the most common pitfalls currently are failed patches that completely mess up the file.
- 2. Fix the memory handling system, and save error logs so future models have refernce.
- 3. Leverage chained LLM calls to optimize context and perform function calls. An example could be lighter weight models (o4-mini) querying the codebase (as it grows larger) or writing patches. Outsource "easy" job to o4-mini and keep heavy reasoning tasks to o3
